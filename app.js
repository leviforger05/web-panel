      ...
      if (response.data.data) {
        ...
        enrichedServers = response.data.data.map(server => {
          ...
          if (!storedPanel) {
            ...
            const newServerEntry = {
              ...
              panel_data: {
                ...
                specs: { ram: ramDisplay, cpu: cpuDisplay, disk: limits.disk }
              }
            };
          ...
          return {
            ...
            specs: storedPanel.panel_data.specs || { ram: ramDisplay, cpu: cpuDisplay }
          };
        });
...
app.post('/admin/addtime', adminAuth, async (req, res) => {
  try {
...
app.post('/admin/deltime', adminAuth, async (req, res) => {
  try {
    const { idserver, days } = req.body;
...
app.post('/admin/suspend', adminAuth, async (req, res) => {
  try {
...
app.post('/admin/unsuspend', adminAuth, async (req, res) => {
  try {
    const { idserver } = req.body;
    if (!idserver) return res.status(400).json({ success: false });
...
app.post('/admin/create-panel', adminAuth, async (req, res) => {
  try {
...
app.post('/admin/api/save-settings', adminAuth, async (req, res) => {
  try {
    const { logo, banner, announcement, announcementActive, maintenanceMode, prices } = req.body;
...
app.post('/api/create-qris', validatePrice, async (req, res) => {
  try {
    const { order_id, username, product_name, days, product_id, type = 'new' } = req.body;
...
app.post('/api/force-create-panel', adminAuth, async (req, res) => {
  try {
...
app.get('/api/panels', adminAuth, (req, res) => {
  try {
...
app.post('/admin/api/panels/delete/:serverId', adminAuth, async (req, res) => {
  try {
...
app.get('/api/panels-database', adminAuth, async (req, res) => {
  try {
    const db = await loadServerDatabase();
    res.json({
      success: true,
...
app.get('/api/server-details/:serverId', adminAuth, async (req, res) => {
  try {
...
app.delete('/admin/api/database-panel/:panelId', adminAuth, async (req, res) => {
  try {
...
async function renewPanel(orderData) {
  try {
    ...
    if (serverIndex === -1 && orderData.username) {
      const serverIndexByUsername = db.findIndex(p => 
        p.username === orderData.username || 
        (p.panel_data && p.panel_data.username === orderData.username)
      );
      if (serverIndexByUsername !== -1) {
        orderData.server_id = db[serverIndexByUsername].panel_data.server_id;
    ...
    try {
      await axios.post(`${config.PTERODACTYL.DOMAIN}/api/application/servers/${orderData.server_id}/unsuspend`, {}, {
        headers: { 'Authorization': `Bearer ${config.PTERODACTYL.API_KEY}`, 'Accept': 'application/json' }
      });
    } catch (e) {}
